...
async function loginSuccess(user) {
  ...
  if (fetchError || !profile) {
    const username = user.user_metadata?.username || (user.email ? user.email.split('@')[0] : user.id);
...
function setTheme(theme) {
  ...
  document.body.classList.remove('theme-dark', 'theme-white', 'theme-dahlia');
  
  // Apply new theme
  if (theme === 'dahlia') {
    document.body.classList.add('theme-dahlia');
  }
  
  // Update all toggle buttons (guard by checking existence first)
  const emoji = theme === 'dahlia' ? '☀️' : '🌙';
  const sidebarBtn = document.getElementById('sidebarThemeToggleBtn');
  const globalBtn = document.getElementById('globalThemeToggleBtn');
  if (sidebarBtn) sidebarBtn.textContent = emoji;
  if (globalBtn) globalBtn.textContent = emoji;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggleButtons = document.querySelectorAll('#sidebarThemeToggleBtn, #globalThemeToggleBtn');
  toggleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const newTheme = currentTheme === 'gray' ? 'dahlia' : 'gray';
      setTheme(newTheme);
    });
  });
});
...
function renderMessages(msgs) {
  ...
  msgs.forEach(msg => {
    ...
    if (isOwn) {
      ...
      senderIdentity = msg.sender.username || 'Other';
    }
